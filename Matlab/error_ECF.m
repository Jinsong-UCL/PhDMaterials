...
tic
dt = T/nsteps;
phi_e_block = zeros(nblocks,npaths,ngrid);
for block = 1:nblocks
    phi_e_path = zeros(1,npaths,ngrid);
    for path = 1:npaths    
        ...
        phi_e_path(1,path,:) = exp(1i*xi*x(end));
        
       % MC_result(1:nsteps+1,path) = S0*exp(x_i_j);    
    end
    phi_e_block(block,:,:) = phi_e_path(1,:,:);
end
phi_e_average = reshape(phi_e_block,[nblocks*npaths ngrid]);
phi_e = mean(phi_e_average,1);
...
fprintf('%22s%14.10f\n','Mean equare erorr',MSE)
fprintf('%22s%14.10f\n','Root mean square erorr',RMSE)
