% Simulation using Gnoatto and Grasselli's numbers
S0 = 1;
K = 1;
    ...
    for path = 1:npaths
        ...
        for step = 1:nsteps
            yPaths(:,:,step+1) = max(yPaths(:,:,step) + (beta*(sigma'*sigma) + kappa*yPaths(:,:,step) + yPaths(:,:,step)*kappa') * dt ...
                +(sqrtm(yPaths(:,:,step))*sigma*dZ(:,:,step) + sigma'*dZ(:,:,step)'*sqrtm(yPaths(:,:,step))'),0);
        end
        ...
        for step = 1:nsteps
            ...
            mu(step) = trace(yPaths(:,:,step) * h + h1 - h2) +  0.5 * (sum_1(step));
        ...
        r_sum = 0;
        for step = 1:nsteps
            r_sum = r_sum + (trace(yPaths(:,:,step) * hm)+h1-h2) *dt;
        end
        dfactor = exp(- r_sum);
