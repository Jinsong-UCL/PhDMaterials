...
% phi_v_q(1,1:ngrid) = 0.5*xi.^2 - 0.5 * ((q^2+ q * a_ij_tilde(1)/a_ij(1))- 1i * xi *(2*q + a_ij_tilde(1)/a_ij(1)));
% phi_v_q(2,1:ngrid) = 0.5*xi.^2 - 0.5 * ((q^2+ q * a_ij_tilde(2)/a_ij(2))- 1i * xi *(2*q + a_ij_tilde(2)/a_ij(2)));

phi_v_q(1,1:ngrid) = 0.5*(xi.^2 - 1i*(2*q-1)*xi - (q-1)*q);
phi_v_q(2,1:ngrid) = 0.5*(xi.^2 - 1i*(2*q-1)*xi - (q-1)*q);
...
zeta_q_v(1,1:ngrid) = 0.5*(4*mu_q_v(1,1:ngrid).^2 + 2*gamma(1)^2*phi_v_q(1,1:ngrid)*a_ij(1)^2).^0.5;
zeta_q_v(2,1:ngrid) = 0.5*(4*mu_q_v(2,1:ngrid).^2 + 2*gamma(2)^2*phi_v_q(2,1:ngrid)*a_ij(2)^2).^0.5;
...
call_option_multiplier = exp(-r_i_0 * T) * exp(q * r_i_0 * T)*S0;
call_option_integrand = ((S0/ K).^(q - 1 - 1i*xi).* exp(-1i*xi*r_i_0*T)) .* underline_W_q_v./ (-xi.^2 - (2*q-1)*xi*1i + q*(q-1));
call_option_integration = sum(call_option_integrand)*dxi/(2*pi); 
