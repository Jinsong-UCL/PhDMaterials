...
ev = (kappav.'*ones(1,ngrid)+(sigmav.*a_ij_rho)'.*(-1i*xi+alpha));
er = (kappar.'*ones(1,ngrid)+(sigmar.*b_ij_rho)'.*(-1i*xi+alpha));
dv= (ev.^2 + diag(sigmav.^2.*a_ij_minus.^2)*fv).^0.5;
dr= (er.^2 + diag(sigmar.^2)*(diag(b_ij_minus.^2)*fr+2*diag(b_ij_division)*ones(2,1)*(alpha-1i*xi))).^0.5; %
gv = (ev - dv)./ (ev + dv);
gr = (er - dr)./ (er + dr);
