...
!! Note that the unit is a day rember that a year is made by 261 daily observations on Bloomberg.
	...
	open(4,file='price_in_sample_call_n.txt',status='unknown')
      open(5,file='param_rec_call_n.txt',status='unknown')
...
!!! e' fondamentale prendere negativo questo tasso         
         ...
         param(12)=0.1d0
           ...
           param(1)=  0.0010000000000000E+000
           param(2)=  0.180308695924283     
           param(3)=  0.103256946684022     
           param(4)=  1.006065913495312E-002
           param(5)=   1.03764723319697     
           param(6)= -0.204225786159639     
           param(7)=  0.0010000000000000E+000
           param(8)=  8.203499101914119E-002
           param(9)=  0.0010000000000000E+000
          param(10)=  0.0010000000000000E+000
          param(11)=  0.0010000000000000E+000
          param(12)=  0.0010000000000000E+000
     
         
         
         
         param( 1 )=  0.2242667567689     
           param(2)=  0.01824205324289206     
          param( 3)=  0.1556841730925742     
           param(4)=  9.938619783173716E-002
          param( 5)=   8.26972091920758     
           param(6)= -0.75108811843432313     
           param(7)=  0.08296509477124     
           param(8)=  7.973025717244276E-003
           param(9)=  1.457559120299295E-003
          param(10)=   6.73402012287118     
          param(11)= -0.904984577783591
          param(12)=  0.997082139138434  
    
              param( 1)=  0.357075566643133     
           param(2)=  0.627044220484031     
           param(3)=  0.159612087552396     
           param(4)=  1.188170051267565d-002
           param(5)=   9.08215196838414     
           param(6)= -0.277523096976147     
           param(7)=  0.191576946141869     
           param(8)=  7.380701729679191d-003
           param(9)=  1.288767408215201d-003
          param(10)=   4.85137913581872     
          param(11)= 0.903415967716359     
          param(12)=   2.12468467201500
            ...
            do j=1,nptime 
               do ik = 1, np
	            ...
	            !  somma=somma+(auxpp-putval(idx(j),ik))**2/putval(idx(j),ik)**2
                    somma=somma+10.d0*(auxpp-putval(idx(j),ik))**2
	     ...
	     sommanew=sommanew+(optvalc(jj,ik)-callval(idx(jj),ik))**2
	             ...
	             ! sommanew=sommanew+(auxop-callval(idx(jj),ik))**2/callval(idx(jj),ik)**2
	              ...
	              !sommanew=sommanew+(auxpp-putval(idx(jj),ik))**2/putval(idx(jj),ik)**2
                     sommanew=sommanew+10.d0*(auxpp-putval(idx(jj),ik))**2
      enddo
	enddo
   ...
   enddo
	...
	do j=1,NN+1
     ...
     !!! start call price computation   
       ...
       !! ho cambiato segno ma non coerente in cdif(2)
    ...
    if(cdabs(zita).gt.0.1d0) then
        !! case interest rate stochastic
        nupu=2.d0*k1(nb)*theta(nb)/epsq
        
	     tildev=4.d0*zita**2*ep*v0(nb)/(sb)**2
        expression=nupu*(cdlog(sb/(2.d0*zita))+(mu+zita)*tau)+v0(nb)*sg*(cdif(nb)/sb)+v0(nb)*tau*(-0.5d0+qq-i*kapp)
    ...
    epsq=epsil(nb)**2
         nupu=k1(nb)*theta(nb)
        !! case interest rate deterministic   
        expression=nupu*zita*tau**2*cdif(nb)+v0(nb)*tau*(cdif(nb)/(1.d0-(zita+mu)*tau))+v0(nb)*tau*(-0.5d0+qq-i*kapp)
        
    svsb=(1.d0-(zita+mu)*tau)**2
    unom=0.5d0*((1.d0-zita*tau)/svsb)
    
    expression=expression+nupu*tau**2*unom+0.5d0*v0(nb)*tau*(1.d0-2.d0*zita*tau)/(svsb*(1.d0+tau*0.5*epsq*unom))
       ...
       phiq=0.5d0*(kapp**2+i*kapp*(2.0d0*qq-1.d0)-(qq**2-qq))
        ...
        cdif(1)=phiq*psii
   !! segno di k in cdif 2 da vedere
        cdif(2)=phiq*omeg**2-qq+i*kapp
