!  Calibratio.f90 
!! Il programma usa la Parity Call Put per calibrare
    !!
    !
!  FUNCTIONS:
...
!! Note that the unit is a day rember that a year is made by 261 daily observations on Bloomberg.
	...
	open(4,file='price_in_sample.txt',status='unknown')
      open(5,file='param_rec.txt',status='unknown')
...
!!! e' fondamentale prendere negativo questo tasso         
         ...
         param( 1 )=  1.42242667567689     
           param(2)=  0.824205324289206     
          param( 3)=  0.556841730925742     
           param(4)=  9.938619783173716E-003
          param( 5)=   5.26972091920758     
           param(6)= -0.108811843432313     
           param(7)=  0.208296509477124     
           param(8)=  7.973025717244276E-003
           param(9)=  1.457559120299295E-003
          param(10)=   3.73402012287118     
          param(11)= -0.904984577783591     
          param(12)=  0.997082139138434*0  
...
!cc calcolo funzione teorica
 ...
 144        call paritycall(nv,np,npmat,nptime,idx,param,texpire_c_p,future_value,Kstrike,optvalc,optvalp)
!cc calcolo funzione
            somma=0.d0
            do j=1,nptime 
               do ik = 1, np
...
!! pulisco i vettori
	 ...
	 call paritycall(nv,np,npmat,nptime,idx,paramnew,texpire_c_p,future_value,Kstrike,optvalc, optvalp)
...
999 continue    
    do j=1,nptime
      do ik = 1, np
	   auxop=optvalc(j,ik)
	   auxpp=optvalp(j,ik)
	   write(4,920) j,future_value(idx(j)),auxop,callval(idx(j),ik), auxpp,optvalp(j,ik),somma
       	   write(*,920) j,future_value(idx(j)),auxop,callval(idx(j),ik), auxpp,optvalp(j,ik),somma
...
!!  e' meglio la formula implementata in L2
      subroutine paritycall(nvv,npp,nppmat,npptime,idxx,par,tmp_c_p,fut,KKstrike,optc,optp)
...
!!!!	   
	  ...
	  do jj=1,NN
         csum=csum+E**(i*kp(jj))*cdexp(-i*kp(jj)*Tm*v0(2)*0.d0)*Ac(jj)*(kp(jj+1)-kp(jj))  
           csump=csump+E**(i*kp(jj))*cdexp(-i*kp(jj)*Tm*v0(2)*0.d0)*Ap(jj)*(kp(jj+1)-kp(jj))  
	  end do
       ...
       !     csum=csum+(fut(idxx(j))-KKStrike(ik))*dexp(-v0(2)*Tm*0.5d0)
            ...
            csump=csump*dexp(-v0(2)*Tm*0.5d0)*KKStrike(ik)**3/(2.d0*pi*fut(idxx(j))**2)
...
nu=2.d0*k1n(2)*thetan(2)/eps(2);
srg=(1.d0-cexp(-k1n(2)*Tm));
Mlam=(4.0*chi_r/(4*chi_r+tmax*eps_r^2*srg));
Bondapp=dexp(-0.5*v0(2)*Tm)*Mlam^nu*dexp(-0.5*Mlam*Tm*v0(2)*dexp(-k1n(2)*Tm));
