...
%dati_all=price_in_sample_call_put_15_02_def3;

load price_in_sample_call_put_15_02_def10.txt
dati_all=price_in_sample_call_put_15_02_def10;


load price_in_sample_call_put_25_02_def10_new.txt
dati_all1=price_in_sample_call_put_25_02_def10_new;


load price_in_sampleH0_call_put_new.txt
dati_allH0=price_in_sampleH0_call_put_new;
...
forecall1=dati_all1(:,3);
obscall1=dati_all1(:,4);

foreput1=dati_all1(:,5);
obsput1=dati_all1(:,6);
...
for is=1:nstr 
      ...
      callfor(is,io)=forecall1(icont);
      errcall(is,io)=abs(obscall1(icont)-forecall1(icont))/abs(obscall1(icont));
      
   callobs(is,io)=obscall1(icont);
   putobs(is,io)=obsput1(icont);
      putfor(is,io)=foreput1(icont);
      errput(is,io)=abs(obsput1(icont)-foreput1(icont))/abs(obsput1(icont));
    end
end 
...
mn1=mean(mean(errcall))
disp('errore medio put one factor')
mn2=mean(mean(errput))

pause
disp('--------------------')
disp('heston')
disp('errore medio call one factor') 
mn3=mean(mean(errcallH0))
disp('errore medio put one factor')
mn4=mean(mean(errputH0))
