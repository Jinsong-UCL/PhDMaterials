...
! nv e' il numero delle variabili
    ...
    real*8 omega
      ...
      integer jalp  
       ...
       open(7,file='param_rec_call_put_25_02_def10.txt',status='unknown')
   !!! parametri ottimi formula trapezi
       ! open(7,file='param_rec_call_put_v1.txt',status='unknown')
              open(4,file='out_sample_call_put_67_84_def10_newP.txt',status='unknown')
          !open(6,file='out_sample_put_67_84.txt',status='unknown')      
!!
...
!      maxdist=264-22
      ...
      enddo
            ...
            maxdist=264-66-22         
!! osserva che il primo tempo e' circa il vecchi 69
      do j=1, npmat
         do ii=1, np
	     texpire_c_p(j,ii)=dfloat(maxdist-(2*(35+j)-1))/264.d0
        enddo
      enddo
...
!! osserva che il primo tempo e' circa il vecchi 69
      do j=1, npmat
         do ii=1, np
	     texpire_c_p(j,ii)=dfloat(maxdist-(2*(j)-1))/264.d0
        enddo
      enddo
...
!      do j=1, npmat
!         do ii=1, np!
	 !    texpire_c_p(j,ii)=dfloat(maxdist-(2*(66+j)-1)+1)/264.d0
     !   enddo
     ! enddo
...
nptime=1
           ...
           param(1)=   1.97937844468695     
           param(2)=  1.827153323790122E-002
           param(3)=  8.910314896890996E-002
           param(4)=  3.450867926827578E-002
           param(5)=  0.656541785689024     
           param(6)= -0.976234138509159     
           param(7)=  9.800155505928972E-003
           param(8)=  2.179902452731032E-004
           param(9)=  4.372799280893374E-004
          param(10)=   3.62862572752895     
          param(11)= -0.809374091815486     
          param(12)=   2.50647032309667     
...
!cc calcolo funzione
           ...
           enddo
      somma=dsqrt(somma)
	write(*,*) 'somma =',somma,' somma scalata=', somma/60.d0
